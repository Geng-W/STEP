...
output_file = rf'/path/to/output/knn_{current_time}_{split_ratio_str}_data.csv'
output_xlsx_file = rf'/path/to/output/knn_{current_time}_{split_ratio_str}_data.xlsx'
...
if use_grid_search:
    model = KNeighborsRegressor(**base_model_params)
    ...
    for i, params in enumerate(param_combinations):
        ...
        test_model = KNeighborsRegressor(**merged_params)
    ...
    best_model = KNeighborsRegressor(**merged_best_params)
    ...
    print(f"Running KNN experiment")
...
if use_grid_search:
    ...
    avg_training_time = np.mean([run['training_time'] for run in all_runs_metrics])  
    avg_testing_time = np.mean([run['testing_time'] for run in all_runs_metrics])   
...
if output_csv:
    with open(output_file, 'w') as f:
        if use_grid_search:
            ...
            f.write("KNN Parameters: n_neighbors=5, weights='uniform', algorithm='auto'\n")
            f.write("Performed 1 run\n")
        f.write("--------------------------------------------------\n")
