...
output_file = rf'/path/to/output/xgb_{current_time}_{split_ratio_str}_data.csv'
output_xlsx_file = rf'/path/to/output/xgb_{current_time}_{split_ratio_str}_data.xlsx'
...
if not XGB_AVAILABLE:
    print("XGBoost library unavailable, program exiting")
    exit()

if use_grid_search:
    model = xgb.XGBRegressor(random_state=1, **base_model_params)
    ...
    for i, params in enumerate(param_combinations):
        ...
        test_model = xgb.XGBRegressor(random_state=1, **merged_params)
        ...
        for fold in range(3):
            ...
            test_model.fit(X_train_fold, y_train_fold, verbose=False)
    ...
    best_model = xgb.XGBRegressor(random_state=1, **merged_best_params)
    ...
    for random_state in range(10):
        ...
        model = xgb.XGBRegressor(random_state=random_state, **base_model_params)
...
if output_csv:
    with open(output_file, 'w') as f:
        if use_grid_search:
            ...
            f.write("XGBoost Parameters: n_estimators=5000, learning_rate=0.01, max_depth=6, subsample=0.8, colsample_bytree=0.8\n")
