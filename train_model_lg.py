...
output_file = rf'/path/to/output/lg_{current_time}_{split_ratio_str}_data.csv'
output_xlsx_file = rf'/path/to/output/lg_{current_time}_{split_ratio_str}_data.xlsx'
...
if use_grid_search:
    model = LinearRegression(**base_model_params)
    ...
    for i, params in enumerate(param_combinations):
        ...
        test_model = LinearRegression(**merged_params)
    ...
    best_model = LinearRegression(**merged_best_params)
...
if output_csv:
    with open(output_file, 'w') as f:
        if use_grid_search:
            ...
            f.write("Linear Regression Parameters: fit_intercept=True\n")
